...
import videoScreen from '../../assets/video-screen.png';
...
const MacBookShowcase = ({ darkMode }) => {
  ...
  const [mobPickup, setMobPickup] = useState("Current Location");
  const [mobDest, setMobDest] = useState("Enter Destination");
  const [bookingDate, setBookingDate] = useState("2026-04-26");
  const [bookingTime, setBookingTime] = useState("21:10");
  ...
  const tabs = [
    { id: 'Local Ride', icon: <MapPin size={18} />, label: 'Local Ride' },
    { id: 'Intercity', icon: <Navigation size={18} />, label: 'Intercity' },
    { id: 'Airport', icon: <Plane size={18} />, label: 'Airport' },
    { id: 'Tour Package', icon: <Landmark size={18} />, label: 'Tour Package' },
  ];
  ...
  return (
    <section className={`py-32 overflow-hidden relative ${darkMode ? 'bg-[#020617]' : 'bg-slate-50'}`}>
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        >
          ...
          <div className="hidden md:block relative mx-auto max-w-[900px] group">
            {/* The Screen / Chassis */}
                ...
                <div className={`p-8 h-full flex flex-col ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                  ...
                  <div className="mb-6">
                    <div className={`inline-flex items-center gap-1.5 ${darkMode ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600'} px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest mb-3`}>
                      <Zap size={10} className="fill-current" /> Smart Booking
                    </div>
                  ...
                  <div className="space-y-4 mb-6">
                    <div className={`group flex items-center gap-3 p-4 rounded-2xl border transition-all ${darkMode ? 'bg-slate-800/50 border-slate-700 focus-within:border-emerald-500/50' : 'bg-white border-slate-100 shadow-sm focus-within:border-emerald-200'}`}>
                      <Search size={18} className="text-slate-400" />
                      <MapPin size={18} className="text-red-500" />
                      <input 
                        type="text" 
                        placeholder="Pickup location" 
                        className="flex-1 bg-transparent border-none outline-none text-xs font-bold placeholder:text-slate-400"
                        value={pickup}
                        onChange={(e) => setPickup(e.target.value)}
                      />
                      <LocateFixed size={18} className="text-emerald-500 cursor-pointer" />
                    </div>
                  ...
                  <button 
                    ...
                    className="w-full py-5 rounded-[1.5rem] bg-gradient-to-r from-emerald-400 to-blue-500 text-white font-black text-sm shadow-xl shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-70"
                  >
                    {isSearching ? (
          ...
          <motion.div 
            ...
            className="md:hidden relative mx-auto w-[280px] z-20 group"
          >
                ...
                <div className="p-5 flex flex-col h-full bg-white overflow-y-auto scrollbar-hide">
                  ...
                  <div className="mb-5">
                    <div className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest mb-2">
                      <Zap size={8} className="fill-current" /> Smart Booking
                    </div>
                    ...
                    {['Local', 'Intercity', 'Airport'].map((type) => (
                      <button
                        ...
                        className={`flex-1 py-2 rounded-lg text-[9px] font-black transition-all ${
                          (activeTab === type || (activeTab === 'Intercity' && type === 'Intercity'))
                  ...
                  <div className="space-y-3 mb-5">
                    <div className="flex items-center gap-2 p-3 rounded-xl border border-slate-100 bg-slate-50">
                      ...
                      <input 
                        type="text" 
                        className="bg-transparent border-none outline-none text-[10px] font-bold text-slate-900 w-full"
                        value={pickup}
                        onChange={(e) => setPickup(e.target.value)}
                      />
                    </div>
                  ...
                  <button 
                    onClick={handleSearch}
                    disabled={isSearching}
                    className="w-full mt-auto bg-gradient-to-r from-emerald-500 to-blue-600 text-white py-4 rounded-2xl font-black text-[11px] shadow-lg shadow-emerald-500/20 active:scale-95 transition-all disabled:opacity-70"
                  >
                    {isSearching ? (
